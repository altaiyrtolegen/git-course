...
export default function AnalysisPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FFFE] text-[#0D2B25] font-sans">
      <Navbar />
      ...
      <div className="pt-20 max-w-7xl mx-auto px-4 sm:px-6 pb-16">
        ...
        <div className="pt-10 pb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-calistoga text-[#0D2B25] mb-2">Рабочая область</h1>
            <p className="text-[#5A8278] font-medium">
          ...
          {mode === "done" && (
            <button
              onClick={handleReset}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-[#C2EDE2] text-[#5A8278] font-medium hover:text-[#0D2B25] hover:border-[#3BBFA3] transition-colors text-sm"
            >
              <RotateCcw size={15} />
              Новый документ
            </button>
            ...
            >
              ...
              <div className="border-b border-[#E7F8F3] px-6 py-4 flex items-center gap-4">
                ...
                <input
                  ...
                  className="flex-1 font-medium text-sm bg-transparent outline-none text-[#0D2B25] placeholder-[#A0C4BA]"
                />
                <button
                  onClick={fillSample}
                  className="text-xs font-medium text-[#3BBFA3] hover:text-[#2A9178] transition-colors whitespace-nowrap"
                >
                  Загрузить пример
                </button>
              </div>
              ...
              <div className="border-t border-[#E7F8F3] px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-2 text-xs font-medium text-[#5A8278]">
                ...
                <div className="flex gap-3 w-full sm:w-auto">
                  ...
                  <button
                    ...
                    className="flex-1 font-medium sm:flex-none inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl border-2 border-[#3BBFA3] text-[#2A9178] text-sm hover:bg-[#E7F8F3] disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                  >
                    <PlusCircle size={16} />
                    Внести в базу
                  </button>
                  ...
                  <button
                    onClick={handleAudit}
                    disabled={!docText.trim() || mode === "loading"}
                    className="flex-1 sm:flex-none inline-flex font-medium items-center justify-center gap-2 px-6 py-3 rounded-xl bg-[#3BBFA3] text-white text-sm hover:bg-[#2A9178] disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-md shadow-[#3BBFA3]/25"
                  >
                    {mode === "loading" ? (
        ...
        <AnimatePresence>
          {mode === "loading" && (
            ...
            >
              <div className="flex items-center font-medium gap-3 text-sm text-[#5A8278]">
            ...
            >
              ...
              {addSuccess && (
                <div className="bg-[#E7F8F3] border border-[#3BBFA3] rounded-xl px-5 py-4 flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-[#0D2B25]">
                      Документ{docTitle ? ` «${docTitle}»` : ""} успешно добавлен в базу
                    </p>
                    <p className="text-xs font-medium text-[#5A8278] mt-0.5">
              ...
              {lastAction === "audit" && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    ...
                    >
                      <div
                        className="text-3xl font-calistoga mb-1"
                        style={{ color: s.color }}
                      >
                        {s.value}
                      </div>
                      <div className="text-xs font-medium text-[#5A8278]">{s.label}</div>
                    </motion.div>
                  ))}
                </div>
              )}
                ...
                <div className="flex border-b border-[#E7F8F3] overflow-x-auto">
                  {tabs.map((tab) => (
                    <button
                      ...
                      className={`flex items-center font-medium gap-2 px-5 py-4 text-sm whitespace-nowrap transition-all border-b-2 ${
                        activeTab === tab.key
                  ...
                  <AnimatePresence mode="wait">
                        ...
                        {MOCK_DUPLICATES.map((item) => (
                          ...
                          >
                            <div className="flex items-start justify-between gap-4 mb-3">
                              <div>
                                <h4 className="text-[#0D2B25] font-semibold text-sm mb-2">{item.title}</h4>
                                <div className="flex flex-wrap gap-2">
                                  {item.laws.map((law) => (
                                    <span
                                      key={law}
                                      className="px-2 py-0.5 rounded-md font-medium bg-white border border-[#BFDBFE] text-xs text-[#3B82F6]"
                                    >
                                      {law}
                                    </span>
                                  ))}
                        ...
                        {MOCK_OUTDATED.map((item) => (
                          ...
                          >
                            <div className="flex items-start justify-between gap-4 mb-1">
                              <h4 className="text-[#0D2B25] font-semibold text-sm">{item.title}</h4>
                              <span
                                className={`text-xs px-2 py-0.5 font-medium rounded-full flex-shrink-0 ${
                                  item.severity === "high"
                            ...
                            <p className="text-xs font-mono text-[#F59E0B] mb-2">{item.date}</p>
                            <p className="text-xs text-[#5A8278] leading-relaxed">
                              {item.description}
                            </p>
                          </div>
                    ...
                    {activeTab === "graph" && (
                      <motion.div
                        ...
                        exit={{ opacity: 0 }}
                      >
                        <p className="text-sm font-medium text-[#5A8278] mb-5">
                ...
                <div className="bg-white rounded-xl border border-[#C2EDE2] px-5 py-4 flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-[#0D2B25] truncate">
                        {docTitle || "Без названия"}
                      </p>
                      <p className="text-xs font-medium text-[#5A8278]">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                      onClick={handleAudit}
                      className="inline-flex items-center font-medium gap-1.5 px-4 py-2 rounded-lg bg-[#3BBFA3] text-white text-xs hover:bg-[#2A9178] transition-colors"
                    >
                      <Zap size={13} />
                      Повторить
                    </button>
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            />
            <div>
              <h4 className="text-[#0D2B25] font-semibold text-sm mb-2">{title}</h4>
              <div className="flex flex-wrap gap-2">
                {laws.map((law) => (
                  <span
                    key={law}
                    className="px-2 py-0.5 rounded-md font-medium bg-white border border-[#FECACA] text-xs text-[#EF4444]"
                  >
                    {law}
                  </span>
                ))}
              ...
              {explanation && (
                <div className="bg-white rounded-lg border border-[#C2EDE2] p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-medium text-[#3BBFA3] uppercase tracking-wide">
